...
cc.Class({
    ...
    _handleBYBPayment: function (data) {
        cc.log("_handleBYBPayment", data.code, data.PrepayID);
        if (data.code == "0") { //成功
            this.tip.show("购买成功");
    ...
    onClickBuy() {
        if (HttpMng.IsActived == "0" || HttpMng.IsActived == null) {
            this.tip.show("当前用户未激活，请前往平台绑定手机完成激活");
            return;
        }
        var _this = this;
        var callback = function(data) {
            if (data != null) {
                if (data.code == 0) {
                    Utils.sdk.pay(data.data.PrepayID, data.data.Nonce, data.data.TimeStamp, data.data.sign);
                } else {
        ...
        //该订单请求链接为测试链接，参数仅供参考
        HttpMng.payorderReq(2, HttpMng.token, cfg.name, cfg.price, callback);
    },
